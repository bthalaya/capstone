...
function APITest() {
  ...
  const handleTestButtonClick = async () => {
    ...
    try {
      const response = await fetch(
        `${serverURL}/api/server-location?companyName=${encodeURIComponent(
          companyName
        )}&reportYear=${reportYear}&reportType=${encodeURIComponent(
          reportType
        )}`
      );
      if (!response.ok) {
  ...
  useEffect(() => {
    if (companyName) {
      const fetchReportYears = async () => {
        const response = await fetch(
          `${serverURL}/api/report-years?companyName=${encodeURIComponent(
            companyName
          )}`,
          {
            method: "GET",
  ...
  useEffect(() => {
    if (companyName && reportYear) {
      const fetchReportTypes = async () => {
        const response = await fetch(
          `${serverURL}/api/report-types?companyName=${encodeURIComponent(
            companyName
          )}&reportYear=${encodeURIComponent(reportYear)}`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
  ...
  return (
    <div>
      ...
      <button onClick={() => handleDownloadFile(serverLocation)}>
        Download File
      </button>
